# các chơi của các nhân vật
# player1:
# di chuyển: w a s d
# attack: r and t
# player2:
# di chuyển: đown,up,right,left
# attack: phím 1 and 2
...
bg_menu = pygame.image.load("assets/images/background/background.jpg").convert_alpha()
...
def options(intro_count, last_count_update, round_over):
    ...
    while running:
        ...
        scaled_bg = pygame.transform.scale(bg_menu, (SCREEN_WIDTH, SCREEN_HEIGHT))
        screen.blit(scaled_bg, (0, 0))
        draw_textt('SELECT PLAYER', score_font, (255, 255, 255), screen, 400, 40)
        mx, my = pygame.mouse.get_pos()
        ...
        if button_3.collidepoint((mx, my)):
            if click:
                if step == 2:
                    step = step + 1
                    fighter_2 = Fighter(2, 700, 310, True, MARTIAL_DATA, martial_sheet, MARTIAL_ANIMATION_STEPS,
                                        sword_fx)

                if step == 1:
                    step = step + 1
                    fighter_1 = Fighter(1, 200, 310, False, MARTIAL_DATA, martial_sheet, MARTIAL_ANIMATION_STEPS,
                                        sword_fx)
        ...
        if button_5.collidepoint((mx, my)):
            if click:
                if step == 2:
                    step = step + 1
                    fighter_2 = Fighter(2, 700, 310, True, NECRO_DATA, necromancer_sheet, NECROMANCER_ANIMATION_STEPS,
                                        magic_fx)

                if step == 1:
                    step = step + 1
                    fighter_1 = Fighter(1, 200, 310, False, NECRO_DATA, necromancer_sheet, NECROMANCER_ANIMATION_STEPS,
                                        magic_fx)
